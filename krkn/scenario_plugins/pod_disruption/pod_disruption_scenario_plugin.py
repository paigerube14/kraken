...
class PodDisruptionScenarioPlugin(AbstractScenarioPlugin):
    ...
    def get_pods(self, name_pattern, label_selector,namespace, kubecli: KrknKubernetes): 
        if label_selector and name_pattern: 
            logging.error('Only, one of name pattern or label pattern can be specified')
        elif label_selector:
            pods = kubecli.select_pods_by_namespace_pattern_and_label(label_selector=label_selector,namespace_pattern=namespace)
        elif name_pattern:
            pods = kubecli.select_pods_by_name_pattern_and_namespace_pattern(name_pattern=name_pattern, namespace_pattern=namespace)
        else: 
    ...
    def killing_pods(self, config: dict, kubecli: KrknKubernetes):
        ...
        pods = self.get_pods(name_pattern,label_selector,namespace, kubecli)
        pods_count = len(pods)
        if len(pods) < kill:
    ...
    def wait_for_pods(
        ...
        while not timeout:
            pods = kubecli.get_all_pod_info(pod_name_pattern=pod_name, label_selector=label_selector,namespace=namespace, field_selector="status.phase=Running")
            print('pods count'+str(pod_count) + " " + str(len(pods[0].items)))
            if pod_count == len(pods[0].items):
                return
